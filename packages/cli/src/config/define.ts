 ...
 *
 * The module declares itself with `defineView({ key, attach, params, actions })`
 * from `@lobu/views` and mounts once with `mountView(view, Component)`; the
 * `attach` line in the file says where it appears, the folder is only a
 * convention. `lobu apply` bundles the module where the project's
 * node_modules exists (relative files and npm deps resolved) and ships source
 * + bundle + extracted metadata; `lobu run` watches the listed files and
 * re-applies on change. Explicit list, no directory auto-discovery — mirroring
 * `connectorFromFile`.
 */
