...
const VIEW_SOURCE = `import { defineView } from "@lobu/views";
...
describe("view desired state", () => {
  test("bundles at load: key, metadata and server-semantics hash", async () => {
    ...
    // inputs (name defaults to key, description to empty, plus the digest of
    // the bundle produced above).
  ...
  test("a module without a view export fails loud at load", async () => {
    const dir = mkProject({
  ...
  test("a leaf-import-only change produces a changed identity + update op (PR2 round-1 F1)", async () => {
    const entry = `import { defineView } from "@lobu/views";
import { LABEL } from "./label";
export const view = defineView({
  key: "board",
  attach: [{ type: "deal" }],
});
export default function Board() { return LABEL; }
`;
    const dir = mkProject({
      "lobu.config.ts": configWithViews("./views/deal/board.tsx"),
      "views/deal/board.tsx": entry,
      "views/deal/label.ts": `export const LABEL = "IMPORT_VERSION_A";\n`,
      "agents/triage/SOUL.md": "triage",
    });
    const { state: first } = await loadDesiredStateFromConfig({ cwd: dir });
    expect(first.views).toHaveLength(1);
    const firstView = first.views[0];
    // Loader/contract parity: the diff key is what the shared function
    // derives from source + normalized metadata + the produced bundle.
    expect(firstView?.contentHash).toBe(
      contentHash(
        firstView?.sourceCode ?? "",
        {
          name: "board",
          description: "",
          attach: [{ type: "deal" }],
    ...
    remote.views = [
      {
        key: "board",
        name: "board",
        content_hash: firstView?.contentHash ?? "",
        attach: [{ type: "deal" }],
        last_writer: "w",
        updated_at: "t",
      },
    ];
    // Change ONLY the imported leaf; the entry source is untouched.
    writeFileSync(
      join(dir, "views", "deal", "label.ts"),
      `export const LABEL = "IMPORT_VERSION_B";\n`
    );
    const { state: second } = await loadDesiredStateFromConfig({ cwd: dir });
    const secondView = second.views[0];
    expect(secondView?.sourceCode).toBe(firstView?.sourceCode);
    expect(secondView?.compiledCode).not.toBe(firstView?.compiledCode);
    expect(secondView?.contentHash).not.toBe(firstView?.contentHash);
    const plan = computeDiff(second, remote, {});
    expect(plan.rows.find((r) => r.kind === "view")?.verb).toBe("update");
    // Reloading without further edits is a noop again.
    const { state: third } = await loadDesiredStateFromConfig({ cwd: dir });
    const settledRemote = emptyRemote();
    settledRemote.views = [
      {
        key: "board",
        name: "board",
        content_hash: secondView?.contentHash ?? "",
        attach: [{ type: "deal" }],
        last_writer: "w",
        updated_at: "t",
      },
    ...
    expect(
      computeDiff(third, settledRemote, {}).rows.find((r) => r.kind === "view")
        ?.verb
    ).toBe("noop");
  });
});
