    ...
    try {
      ...
      writeFileSync(
        ...
        [
          ...
          `    attempts.push(text);`,
          `    let artifact, error;`,
          `    try { artifact = await bundleViewFromFile(entry, project); } catch (e) { error = e; }`,
          ...
          `    const disk = await bundleViewFromFile(entry, project);`,
          ...
          `  cAttempts.push(text);`,
          `  const artifact = await bundleViewFromFile(cEntry, cProject);`,
