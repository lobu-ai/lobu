  ...
  try {
    while (!options.signal.aborted && !options.stopped()) {
      ...
      await new Promise<void>((resolve) => {
        ...
        // Covers scheduled work, missed notifications and producers older than
        // this build. Deliberately NOT the durable queue's 200ms cadence: that
        // is one loop per replica, while this ticks per in-flight held request
        // and each empty attempt re-runs the claim's due-feed materialization.
        const timer = setTimeout(done, Math.min(remaining, intervals.workerHoldRecheckMs));
        wake = done;
