...
describe("enqueued payload carries the resolved nixConfig", () => {
  ...
  test("routePlatformMessage binds the turn to the AUTHENTICATED caller, not a token digest", async () => {
    // RED before the fix: the session/enqueue userId was
    // `${platform}-${token.slice(0, 8)}` — an identity derived from the token
    // bytes. A blocked tool call stores that as the pending approval's
    // claimant, but the approver presents `authContext.userId`, so the
    // predicate could never match and the approval was unclaimable.
    const sessions: Array<{ userId: string }> = [];
    const manager = Object.create(
      ChatInstanceManager.prototype,
    ) as ChatInstanceManager;
    ...
    // The old synthetic shape must be gone entirely: no token-derived digest.
    expect(enqueued[0]!.userId).not.toBe("telegram-token-ab");
    expect(enqueued[0]!.userId).not.toContain("token-ab");
  });
