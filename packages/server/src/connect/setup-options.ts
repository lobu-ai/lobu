...
export async function publicSetupOptions(
  connectorKey: string,
  origin: string,
  organizations?: readonly OrgInfo[]
): Promise<ConnectionSetupOptions> {
  ...
  if (method?.installShape === 'oauth-code-exchange' && creds?.clientId && creds.clientSecret) {
    options.push({
      kind: 'hosted_chat',
      label: 'Use the hosted Lobu app',
      description:
        'Chat with an agent in Lobu Cloud. This does not connect an independent local runtime or sync its messages locally.',
      execution: 'cloud',
      configured: true,
      url: new URL(`/lobu/${encodeURIComponent(method.provider)}/install`, origin).toString(),
      instructions:
        'Use your cloud workspace and cloud agent. Complete app installation, then bind the intended DM or channel with a Lobu link code. Cloud device execution still uses cloud memory.',
    });
  }
  return {
    ...
    options: options.slice(0, MAX_SETUP_OPTIONS),
  };
}

/** A trusted operator-selected origin; never forward caller cookies or tokens. */
export async function fetchCloudSetupOptions(
  connectorKey: string,
  origin: string,
  ...
  if (
    !Value.Check(ConnectionSetupOptionsSchema, result) ||
    result.connector_key !== connectorKey ||
    result.cloud_status !== 'available'
  )
    throw new Error('Invalid cloud setup metadata');
  for (const option of result.options) {
    if (
      option.kind === 'local' ||
      !option.configured ||
      !option.url ||
      new URL(option.url).origin !== new URL(origin).origin
    )
      throw new Error('Invalid cloud setup option');
  }
  return result;
}
...
export interface SetupOptionsDeps {
  ...
  localOption(
    ...
    connectorKey: string,
    origin: string
  ): Promise<ConnectionSetupOption | null>;
}

export async function localSetupOption(
  organizationId: string,
  connectorKey: string,
  origin: string
): Promise<ConnectionSetupOption | null> {
  ...
  const rows = await sql`
    SELECT d.key AS installed_key, d.auth_schema, o.slug
    FROM "organization" o
    LEFT JOIN connector_definitions d
      ON d.organization_id = o.id AND d.key = ${connectorKey} AND d.status = 'active'
  ...
  const app = rows[0].installed_key
    ? getAppInstallationAuthMethods(schema)[0]
  ...
  const setupUrl = new URL(
    `/${encodeURIComponent(rows[0].slug)}/connectors/${encodeURIComponent(connectorKey)}`,
    origin
  );
  ...
  if (app) {
    ...
    configured =
      app.installShape === 'oauth-code-exchange'
        ? !!(creds.clientId && creds.clientSecret)
        : !!(creds.appId && creds.privateKey && creds.appSlug);
    if (configured)
      url = new URL(
        `/lobu/${encodeURIComponent(app.provider)}/${app.installShape === 'github-app' ? 'app/install' : 'install'}`,
        origin
      ).toString();
  } else if (oauth) {
    const profiles = await sql`
      ...
      WHERE organization_id = ${organizationId}
        AND profile_kind = 'oauth_app' AND status = 'active'
        AND (connector_key = ${connectorKey} OR lower(provider) = ${oauth.provider.toLowerCase()})
      LIMIT 1
    `;
  ...
  return {
    kind: 'local',
    label: configured ? 'Use this server’s app' : 'Use your own app',
    description: configured
      ? 'Connect through this Lobu server.'
      : 'Configure an app on this server if you prefer to manage its credentials.',
    execution: 'local',
    configured,
    url,
    instructions: configured
      ? 'Continue setup on this server.'
      : 'An administrator must configure the app before local authorization is available.',
  };
}

const DEFAULT_DEPS: SetupOptionsDeps = {
  cloudOrigin: resolveCloudOrigin,
  cloudMode: isCloudMode,
  publicOptions: publicSetupOptions,
  remoteOptions: fetchCloudSetupOptions,
  localOption: localSetupOption,
};

export async function connectionSetupOptions(
  connectorKey: string,
  organizationId: string,
  origin: string,
  overrides: Partial<SetupOptionsDeps> = {}
): Promise<ConnectionSetupOptions> {
  const deps = { ...DEFAULT_DEPS, ...overrides };
  const local = await deps.localOption(organizationId, connectorKey, origin);
  let result: ConnectionSetupOptions = {
    action: 'setup_options',
    connector_key: connectorKey,
