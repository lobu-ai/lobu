...
import {
  ...
  fetchCloudSetupOptions,
  type SetupOptionsDeps,
} from '../setup-options';
import type {
...
const managed: ConnectionSetupOption = {
  kind: 'managed_oauth',
  label: 'Connect with Example',
  description: 'Managed OAuth',
  execution: 'local',
  configured: true,
  managed_by_org: 'public-provider',
  url: 'https://cloud.example/connect/managed?org=public-provider&connector=mail',
  instructions: 'Consent, then bootstrap locally.',
};
const local: ConnectionSetupOption = {
  kind: 'local',
  label: 'Use your own app',
  description: 'Local setup',
  execution: 'local',
  configured: false,
  url: 'http://localhost:8787/test/connectors/mail',
  instructions: 'Configure app',
};
const response = (options = [managed]): ConnectionSetupOptions => ({
...
function deps(overrides: Partial<SetupOptionsDeps> = {}): SetupOptionsDeps {
  return {
    cloudOrigin: async () => 'https://cloud.example',
    cloudMode: () => false,
    publicOptions: async () => response(),
    remoteOptions: async () => response(),
    localOption: async () => local,
    ...overrides,
  };
}
...
describe('shared connection setup discovery', () => {
  test('fresh local runtime offers managed OAuth before BYO without a cloud login', async () => {
    const result = await connectionSetupOptions(
      'mail',
      'local-org',
      'http://localhost:8787',
      deps()
  ...
  test('reserves room for local setup when cloud discovery fills the response limit', async () => {
    const result = await connectionSetupOptions(
      ...
      'local-org',
      'http://localhost:8787',
      deps({ remoteOptions: async () => response(Array.from({ length: 100 }, () => managed)) })
    );
  ...
  test('cloud gateway reads its own public offers without remote recursion', async () => {
    ...
    expect(result.cloud_status).toBe('available');
  });
  test('hosted chat retains cloud execution boundary', async () => {
    ...
    const result = await connectionSetupOptions(
      ...
      'org',
      'http://localhost:8787',
      deps({ remoteOptions: async () => response([chat]) })
    );
...
describe('public cloud metadata transport', () => {
  test('never sends user credentials and forbids redirects', async () => {
    ...
    expect(
      (await fetchCloudSetupOptions('mail', 'https://cloud.example', transport)).options
    ).toEqual([managed]);
  });
  test('rejects cross-origin action URLs and unexpected secrets', async () => {
    for (const option of [
      { ...managed, url: 'https://evil.example/consent' },
      { ...managed, token: 'should-not-be-returned' },
    ]) {
      await expect(
        fetchCloudSetupOptions('mail', 'https://cloud.example', (async () =>
          Response.json(response([option]))) as typeof fetch)
      ).rejects.toThrow();
    }
  });
  test('rejects mismatched connector and overlarge bodies', async () => {
    await expect(
      fetchCloudSetupOptions('other', 'https://cloud.example', (async () =>
...
describe('setup-required enrichment', () => {
  test('optional discovery failure preserves an actionable setup continuation', async () => {
    const continuation = {
      ...
      instructions: 'Configure your OAuth app.',
      next_action: 'configure_oauth_app',
    } as Parameters<typeof withSetupOptions>[0];
    const result = await withSetupOptions(
      continuation,
      'mail',
      { organizationId: 'synthetic-org' } as Parameters<typeof withSetupOptions>[2],
      async () => {
        throw new Error('Discovery database unavailable');
      }
    );
...
describe('cloud discovery origin', () => {
  test('normalizes HTTP origins and rejects unsafe or malformed configured URLs', async () => {
    const previous = process.env.LOBU_CLOUD_URL;
    try {
      for (const [value, expected] of [
        ['https://cloud.example/path', 'https://cloud.example'],
        ['http://localhost:8787/api', 'http://localhost:8787'],
        ['file:///etc/passwd', null],
        ['https://user:pass@cloud.example', null],
        ['ftp://cloud.example', null],
        ['not-a-url', null],
      ] as const) {
        process.env.LOBU_CLOUD_URL = value;
        expect(await resolveCloudOrigin()).toBe(expected);
      }
    } finally {
      if (previous === undefined) delete process.env.LOBU_CLOUD_URL;
      else process.env.LOBU_CLOUD_URL = previous;
    }
  });
});
