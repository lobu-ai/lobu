...
async function withSetupOptions(result: ManageConnectionsResult, connectorKey: string, ctx: ToolContext): Promise<ManageConnectionsResult> {
  if (!('status' in result) || result.status !== 'setup_required') return result;
  const setup_options = await handleSetupOptions({ connector_key: connectorKey }, ctx);
...
const manageConnectionsTool = defineActionTool("manage_connections", {
	...
	setup_options: action(SetupOptionsAction, handleSetupOptions),
