...
export async function handleListConnectorGroups(
  ...
  `;
	...
	for (const row of rows) {
		...
		if (firstConnection)
			connectionIdByKey.set(connectorKey, firstConnection.id);
	}
...
export async function handleList(
  ...
  `;
	...
	return {
		action: "list",
		connections: connections.map((row) => projectConnectionForReader(row, ctx)),
		total,
		limit,
		offset,
...
export async function handleCreate(
	...
	const appInstallGuard = explicitlyNoAuth
		? null
	...
	const interactiveMethod = explicitlyNoAuth
		? null
	...
	if (authSelection?.authProfile) {
		const ownershipError = oauthAccountOwnershipError(
			authSelection.authProfile,
			ctx.userId,
			effectiveCreatedBy,
		);
	...
	if (
		explicitlyNoAuth &&
		(explicitConfig.managedBy ||
			explicitConfig.installation_ref ||
			explicitConfig.consent_only)
	) {
		return {
			error:
				"No-auth selection cannot be combined with delegated or app-installation credentials. Create a separate connection.",
		};
	}
...
export async function handleUpdate(
  ...
  `;
	...
	if (
		...
		!callerIsAdmin &&
		(args.app_auth_profile_slug !== null ||
			existing.app_auth_profile_id !== null)
	) {
	...
	if (
		explicitlyNoAuth &&
		(retainedConfig.managedBy ||
			retainedConfig.installation_ref ||
			retainedConfig.consent_only)
	) {
		return {
			error:
				"Delegated and app-installation connections cannot switch to no-auth. Create a separate connection.",
		};
	}
	const authSelection = await resolveConnectionAuthSelection({
		...
		authProfileSlug: hasAuthProfileArg
			? args.auth_profile_slug
			: currentAuthProfile?.slug,
		appAuthProfileSlug: hasAppAuthProfileArg
			? args.app_auth_profile_slug
			: currentAppAuthProfile?.slug,
		deviceWorkerId: nextDeviceWorkerId,
	});

	if (
		args.auth_profile_slug === null &&
		!explicitlyNoAuth &&
		(authSelection.oauthMethod ||
			authSelection.envMethod ||
			authSelection.browserMethod)
		...
		return {
			error:
				"This connector requires an auth profile. Select a replacement profile, or explicitly select a declared no-auth method by clearing both profile fields.",
		};
	}
	...
	try {
      ...
      `;
			...
			if (
				...
				!callerIsAdmin &&
				args.app_auth_profile_slug === null &&
				lockedRows[0].app_auth_profile_id !== null
			) {
				return {
					denial: { error: "Only admins can clear the OAuth app profile." },
				};
			}
			...
			if (
				explicitlyNoAuth &&
				(lockedResultingConfig.managedBy ||
					lockedResultingConfig.installation_ref ||
					lockedResultingConfig.consent_only)
			) {
				return {
					denial: {
						error:
							"No-auth selection cannot retain delegated or app-installation credentials. Create a separate connection.",
					},
				};
			}
      ...
      `;
			if (explicitlyNoAuth && rows[0]?.status === "active") {
				const feedsSchema = existing.feeds_schema as Record<
					string,
					FeedDefinition
				> | null;
				const syncKeys = Object.keys(feedsSchema ?? {}).filter((key) =>
					feedOperations(feedsSchema, key).includes("sync"),
				);
	...
	if (
		...
		(!explicitlyNoAuth && effectiveStatus !== null)
	) {
		await sql`
    UPDATE feeds
  ...
  `;
	...
	if (
		...
		(nextAuthProfileId !== existing.auth_profile_id ||
			args.status !== undefined) &&
		(effectiveAuth.status === "active" ||
			effectiveAuth.status === "pending_auth")
	) {
...
export async function handleDelete(
  ...
  `;
		const entityIds = affectedFeeds
			.map((row: { entity_id: number | string | null }) =>
