 ...
 * correctness.
 */
export function eventDedupLockKey(
	connectionId: number,
	originId: string,
): number {
	let hash = 0x811c9dc5;
...
function isSemanticallyEqual(
	...
	return (
		...
		stableJson(existing.payload_data ?? {}) ===
			stableJson(params.payloadData ?? {}) &&
		stableJson(existing.payload_template ?? null) ===
			stableJson(params.payloadTemplate ?? null) &&
		...
		durableMetadataState(existing.metadata) ===
			durableMetadataState(params.metadata) &&
		...
		(existing.interaction_status ?? null) ===
...
async function applyVolatileState(
	...
	const metadataChanged =
		stableJson({ ...existingVolatile, ...patch }) !==
		stableJson(existingVolatile);
...
function isEventsClientIdForeignKeyViolation(error: unknown): boolean {
	...
	return (
		...
		(typeof err.message === "string" &&
			err.message.includes("events_client_id_fkey"))
	);
}
...
export async function insertEvent(
	...
	const hasDistinctSourceOrigin =
		browserConnector && sourceOriginId !== params.originId;
	...
	const entityIdsValue =
		params.entityIds.length > 0 ? `{${params.entityIds.join(",")}}` : null;
	...
	const runInsert = async (activeSql: DbClient): Promise<InsertedEvent> => {
		...
		if (options?.onConflictUpdate) {
			let existing = await findCurrentEventByOrigin(
				activeSql,
				sourceOriginLookupParams,
			);
			const existingHasRawBrowserOrigin =
				existing != null && hasDistinctSourceOrigin;
			...
			if (existing) {
				const existingHasRawBrowserParent =
					browserConnector &&
					sanitizeBrowserText(existing.origin_parent_id) !==
						existing.origin_parent_id;
				if (
...
export async function insertConnectionlessWorkspaceEvent(
	...
	try {
		...
		if (!isUniqueViolation(error, AUDIT_IDEMPOTENCY_INDEX) || options?.sql)
			throw error;
...
export interface ConfigChangeEventParams
	extends StateChangeEventParams<ConfigResourceKind> {}
...
export function recordWorkspaceChangeEvent(
	params: WorkspaceChangeEventParams,
): void {
