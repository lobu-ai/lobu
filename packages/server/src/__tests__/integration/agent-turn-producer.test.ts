 ...
 */
function claudeModule(
	overrides: Partial<ModelProviderModule> = {},
): ModelProviderModule {
	return {
		providerId: "claude",
		sdkCompat: "anthropic",
...
function mcpFixture(
	options: { fail?: boolean; serverId?: string } = {},
): McpFixture {
	const listed: McpFixture["listed"] = [];
	const configService = {
		getMcpStatus: async () => [
			{
				id: options.serverId ?? "lobu-memory",
				name: "lobu-memory",
				requiresAuth: false,
				requiresInput: false,
			},
...
async function admittedMessage(
...
async function enqueueMessage(
	message: MessagePayload,
	deps: Parameters<typeof enqueueAgentTurn>[1],
) {
	...
	return {
		...
		read: async (artifactId: string) => {
			...
			return fixture && metadata
				? ({ metadata, bytes: fixture.bytes } as never)
				: null;
		},
	};
}
...
async function threadResponsesGlobal(): Promise<
...
async function postAsFleet(path: string, body: Record<string, unknown>) {
	return post(path, {
		body:
			path === "/api/workers/complete-agent-turn" && body.status === "completed"
				? { consumed_inputs: [], ...body }
				: body,
		token: "test-fleet-token",
		env: { WORKER_API_TOKEN: "test-fleet-token" },
	});
	...
	entries: Array<Record<string, unknown>> = [
		{
			...
			message: {
				role: "user",
				content: "what is the isolate lane?",
				timestamp: 1,
			},
		},
		{
			...
			message: {
				role: "assistant",
				content: [{ type: "text", text: "the isolate answer" }],
				timestamp: 2,
			},
		},
	],
...
describe("agent turn producer", () => {
	...
	it("EXECUTES a real HTTP interaction live with the producer-minted turn credential", async () => {
		...
		try {
			...
			const response = await fetch(
				`http://127.0.0.1:${(server.address() as AddressInfo).port}/internal/interactions/create`,
				{
					method: "POST",
					headers: {
						authorization: `Bearer ${run.action_input.credential}`,
						"content-type": "application/json",
					},
					body: JSON.stringify({
						interactionType: "link_button",
						url: "https://example.invalid",
						label: "Interaction attempt",
					}),
				},
			);
			...
			expect(
				verifyWorkerToken(run.action_input.credential as string),
			).toMatchObject({
				runId: run.id,
			...
			expect(
				(
					verifyWorkerToken(run.action_input.credential as string) as {
						executionMode?: string;
					}
				).executionMode,
			).toBeUndefined();
			const [captured] =
				await getTestDb()`SELECT dry_run_preview FROM runs WHERE id = ${run.id}`;
	...
	it("CAPTURES side effects for a dry-run chat turn instead of performing them", async () => {
		...
		try {
			...
			const sideEffects =
				(
					captured?.dry_run_preview as {
						side_effects?: Array<{ action: string }>;
					} | null
				)?.side_effects ?? [];
			expect(sideEffects.map((entry) => entry.action)).toContain(
				"interactions.create",
			);
		} finally {
			await new Promise<void>((resolve, reject) =>
	...
	it.each([
		"compatible",
		"codex",
	])("executes a %s tool round trip through worker HTTP and an isolate", async (protocol) => {
		...
		const server = createServer(async (req, res) => {
			try {
				...
				if (
					path.startsWith("/lobu/api/proxy/compatible/") ||
					path.startsWith("/lobu/api/proxy/openai-codex/")
				) {
					requests.push(body);
					if (protocol === "codex") {
						expect(path.endsWith("/codex/responses")).toBe(true);
						expect(
							verifyWorkerToken(req.headers.authorization!.slice(7)).agentId,
						).toBe(AGENT_ID);
					...
					if (Object.hasOwn(body, "store")) {
						...
						res.end(
							JSON.stringify({ error: { message: "Unknown name 'store'" } }),
						);
						return;
					}
								...
								tool_calls: [
									{
										...
										function: {
											...
											arguments: JSON.stringify({
												file_path: "probe.txt",
												content: "compatibility verified",
											}),
										},
									},
								],
		...
		try {
			...
			message.agentOptions!.model =
				protocol === "codex"
					? "chatgpt/gpt-5.6-luna"
					: "compatible/compatible-model";
			await enqueueMessage(message, {
				agentSettings: settingsStore,
				gatewayUrl: `${origin}/lobu`,
				catalog: catalogFor(
					protocol === "codex"
						? new ChatGPTOAuthModule({} as never)
						: claudeModule({
								providerId: "compatible",
								sdkCompat: "openai",
								getUpstreamConfig: () => ({
									slug: "compatible",
									upstreamBaseUrl: "https://compatible.example.test/v1",
								}),
								getProxyBaseUrlMappings: () => ({
									OPENAI_BASE_URL: `${origin}/lobu/api/proxy/compatible/a/turn-agent`,
								}),
							}),
				),
			});
			...
			const client = new WorkerClient({
				apiUrl: origin,
				workerId: "synthetic-compat-worker",
				authToken: "synthetic-compat-fleet",
			...
			if (protocol === "codex") {
				...
				expect(
					requests.every((request) => !Object.hasOwn(request, "store")),
				).toBe(true);
				expect(
	...
	it("executes write/edit over worker HTTP with admitted history", async () => {
		const calls = [
			{
				name: "write",
				input: { file_path: "a.txt", content: "\ufeffbefore\r\n" },
			},
			{
				name: "edit",
				input: {
				...
				if (path.startsWith("/lobu/api/proxy/anthropic/")) {
					...
					send("content_block_delta", {
						...
						delta: call
							...
							: {
									type: "text_delta",
									text: "File edited; failed edit preserved its contents.",
								},
					});
			...
			const history = nativeSession([
				{
					...
					message: {
						role: "user",
						content: "discarded-branch-history",
						timestamp: 1,
					},
				},
				{
				...
				},
				{
					...
					message: {
						role: "user",
						content: "retained-branch-history",
						timestamp: 2,
					},
				},
			]);
      ...
      `;
			...
			const client = new WorkerClient({
				apiUrl: origin,
				workerId: "fleet-file-tools",
				authToken: "test-file-tools-fleet",
				capabilities: { agent_turn: true },
			});
			const job = await client.poll();
			expect(job.run_id).toBe(Number(run.id));
			expect(
				(job.payload as { turn: { session_jsonl: string } }).turn.session_jsonl,
			).toBe(history);
			const result = await executeRun(
				client,
				job,
			...
			expect(JSON.stringify(providerRequests[0].messages)).toContain(
				"retained-branch-history",
			);
			expect(JSON.stringify(providerRequests[0].messages)).not.toContain(
				"discarded-branch-history",
			);
			expect(
				providerRequests[0].tools.find((tool) => tool.name === "edit")
					?.input_schema,
			).toMatchObject({
				required: ["file_path", "old_string", "new_string"],
			});
			expect(workerRequests.map((request) => request.path)).toContain(
				"/api/workers/heartbeat",
			);
			const completion = workerRequests.find(
				(request) => request.path === "/api/workers/complete-agent-turn",
			)!.body;
			expect(completion.status).toBe("completed");
			expect((await runRow(Number(run.id))).status).toBe("completed");
			const [snapshot] =
				await sql`SELECT snapshot_jsonl FROM agent_transcript_snapshot WHERE run_id = ${run.id}`;
			expect(completion.session_jsonl.startsWith(history)).toBe(true);
			expect(snapshot.snapshot_jsonl).toBe(completion.session_jsonl);
			expect(
				(await runRow(Number(run.id))).action_input.result?.session_jsonl,
			).toBe(completion.session_jsonl);
			const messages = parseSessionEntries(completion.session_jsonl)
			...
			expect(toolResults[2].content[0].text).toContain(
				"Could not find the exact text in a.txt",
			);
			// Pi's read preserves the BOM as well as the CRLF bytes retained by edit.
			expect(toolResults[3].content).toEqual([
				{ type: "text", text: "\ufeffafter\r\n" },
			]);
			expect(toolResults[4].content).toEqual([
				{
					type: "text",
			...
			const [reply] =
				await sql`SELECT action_input FROM runs WHERE queue_name = 'thread_response' AND action_input->>'finalText' = ${completion.text}`;
			...
			const snapshots =
				await sql`SELECT id FROM agent_transcript_snapshot WHERE run_id = ${run.id}`;
	...
	it.each([
		...
		"late",
		"cancel",
	] as const)("preserves accepted input across real HTTP and isolates (%s)", async (timing) => {
		const org = await createTestOrganization();
		const sql = getTestDb();
		const first = messageFor(org.id);
		const followUp = {
			...first,
			messageId: "http-follow-up",
			messageText: "durable-http-follow-up",
		};
		...
		const enqueue = async (message: MessagePayload) => {
			...
			if (!(await cancelAgentTurn(source)))
				await enqueueAgentTurn(source, {
					agentSettings: settingsStore,
					catalog: catalogFor(tokenEchoingModule()),
					gatewayUrl: `${origin}/lobu`,
		...
		const server = createServer(async (req, res) => {
			try {
				...
				if (path.startsWith("/lobu/api/proxy/anthropic/")) {
					...
					providerRequests.push(body);
					if (step > 1)
						throw new Error("duplicate or unexpected provider execution");
					res.writeHead(200, { "content-type": "text/event-stream" });
					const send = (type: string, data: unknown) =>
						res.write(`event: ${type}\ndata: ${JSON.stringify(data)}\n\n`);
					send("message_start", {
					...
					send("content_block_delta", {
						...
						delta: {
							type: "text_delta",
							text: step ? "Follow-up answered." : "Initial answer.",
						},
					});
				...
				if (path.endsWith("/complete-agent-turn")) {
					...
					if (timing === "mid" && !droppedCompletion) {
		...
		try {
			...
			const client = new WorkerClient({
				apiUrl: origin,
				workerId: "fleet-http-input",
				authToken: "test-input-fleet",
				capabilities: { agent_turn: true },
			});
			...
			client.heartbeat = async (...args) => {
				...
				if (response.steer?.length && ++receivedOffers === 2)
					setTimeout(releaseProvider, 0);
				return response;
			};
			const real = new IsolateExecutor({
				allowedDomains: ["127.0.0.1"],
				timeoutMs: 15_000,
			});
			let executions = 0;
			const executor: SyncExecutor = {
				execute: async (...args) => {
					...
					try {
						return await real.execute(...args);
					} finally {
						if (execution === 1) {
			...
			if (timing === "cancel") {
				...
				expect(
					(
						await (
							await pollFleet("fleet-before-cleanup", { agent_turn: true })
						).json()
					).run_id,
				).toBeUndefined();
			...
			if (timing === "mid") {
				...
				const [row] =
					await sql`SELECT run_metadata, action_input FROM runs WHERE id = ${followerId}`;
				const completion = completions[0].request;
				expect(completion.consumed_inputs).toEqual([
					{
						run_id: followerId,
						session_entry_id: row.run_metadata.session_entry_id,
					},
				]);
				expect(row.run_metadata.consumed_by_run_id).toBe(ownerId);
				expect(row.action_input).not.toHaveProperty("result");
				const entry = completion.session_jsonl
					.trim()
					.split("\n")
					.map((line: string) => JSON.parse(line))
					.find(
						(entry: { id: string }) =>
							entry.id === row.run_metadata.session_entry_id,
					);
				expect(entry.message).toMatchObject({
					role: "user",
					content: [{ type: "text", text: followUp.messageText }],
				});
				expect(completions.at(-1)!.response).toMatchObject({
					status: "completed",
					idempotent: true,
				});
			} else {
				...
				const history = (
					successor.payload as { turn: { session_jsonl: string } }
				).turn.session_jsonl;
				if (timing === "late") expect(history).toContain("Initial answer.");
				else expect(history).toBe("");
				expect(
					(await executeRun(client, successor, {}, config)).error,
				).toBeUndefined();
			...
			expect(
				JSON.stringify(providerRequests[1].messages).split(
					followUp.messageText,
				),
			).toHaveLength(2);
			const replies =
				await sql`SELECT action_input FROM runs WHERE queue_name = 'thread_response' AND action_input->'processedMessageIds' ? 'http-follow-up'`;
			expect(replies).toHaveLength(1);
			if (timing === "mid")
				expect(replies[0].action_input.processedMessageIds).toEqual([
					first.messageId,
					followUp.messageId,
				]);
	...
	it("produces a claimable, schema-valid envelope with the credential lifted off the turn", async () => {
		...
		expect(
			Value.Check(AgentTurnPollPayloadSchema, { turn: envelope.turn }),
		).toBe(true);
	...
	it("omits file-delivery instructions when upload_file is denied", async () => {
		...
		expect(turn.system_prompt).not.toContain(
			"call upload_file before the turn ends",
		);
	});
    ...
    `;
		...
		const turn = runs[runs.length - 1]!.action_input.turn as {
			skills?: Array<{ name: string; content: string }>;
		};
		expect(turn.skills).toEqual([
			{ name: "frozen", content: "Frozen instructions." },
		]);
	});
	...
	it("does not advertise seeded inputs when a strict bash policy rejects cat", async () => {
		...
		message.platformMetadata = {
			files: [
				{ id: "art-doc", name: "report.pdf", mimetype: "application/pdf" },
			],
		};
		message.agentOptions = {
		...
		expect(turn.message_files[0].data).toBe(
	...
	it("hands the turn its tools, its one credential being a worker token both gateway routes accept", async () => {
		...
		expect(
			envelope.turn.system_prompt.endsWith(
	...
	it.each([
		...
		{
			policy: { strictMode: true, allowedTools: ["query_sdk"] },
			missing: "run_sdk",
		},
		{
			policy: { allowedTools: ["*"], deniedTools: ["run_sdk"] },
			missing: "run_sdk",
		},
	])("rejects a window before inference when policy excludes $missing", async ({
		policy,
		missing,
	}) => {
		...
		expect(result).toEqual({
			error: expect.stringContaining(`tool policy excludes ${missing}`),
		});
	...
	it("distinguishes failed Automation discovery from excluded tools", async () => {
		const result = await enqueueMessage(await automationMessage(), {
			agentSettings: settingsStore,
			catalog: catalogFor(tokenEchoingModule()),
			mcp: mcpFixture({ fail: true }).mcp,
			gatewayUrl: GATEWAY_URL,
		});
		expect(result).toEqual({
			error: expect.stringContaining("tool discovery failed"),
		});
		expect(await agentTurnRuns()).toEqual([]);
	});
	...
	it.each([
		"absent",
		"other server",
	] as const)("refuses a missing memory mount (%s) even though other tools are available", async (kind) => {
		const result = await enqueueMessage(await automationMessage(), {
			agentSettings: settingsStore,
			catalog: catalogFor(tokenEchoingModule()),
			gatewayUrl: GATEWAY_URL,
			...(kind === "other server"
				? { mcp: mcpFixture({ serverId: "synthetic-other" }).mcp }
				: {}),
		});
		expect(result).toEqual({
	...
	it.each([
		"ordinary",
		"turn",
	] as const)("preserves intentional tool-free %s execution", async (kind) => {
		const message =
			kind === "turn"
				? await automationMessage("turn")
				: messageFor((await createTestOrganization()).id);
		message.agentOptions = {
			...message.agentOptions,
	...
	it("admits a window with explicitly eligible memory tools", async () => {
		...
		message.agentOptions = {
			...message.agentOptions,
			toolsConfig: { strictMode: true, allowedTools: ["query_sdk", "run_sdk"] },
		};
		expect(
			await enqueueMessage(message, {
				agentSettings: settingsStore,
		...
		const [run] = await agentTurnRuns();
		expect(
			run.action_input.turn.tools.definitions.map(
				(tool: { name: string }) => tool.name,
			),
		).toEqual(["query_sdk", "run_sdk"]);
	});
	...
	it("filters the tools through the agent tool policy", async () => {
		...
		expect(turn.tools?.definitions.map((tool) => tool.name)).toEqual([
			"query_sdk",
		]);
	...
	it("carries the bash prefix policy with the workspace, and drops the tools the policy denies", async () => {
		...
		expect(turn.tools?.builtin).toEqual([
			"bash",
			"read",
			"edit",
			"grep",
	...
	it("runs the turn without tools when it cannot honour them, and still enqueues it", async () => {
		...
		const toolless = async () => {
			...
			const turn = rows[produced - 1].action_input.turn as {
				tools?: { definitions: unknown[]; builtin?: string[] };
			};
		...
		const extraTurn = extraRows[produced - 1].action_input.turn as {
			tools?: { definitions?: Array<{ name: string }> };
		};
		expect(extraTurn.tools?.definitions?.map((tool) => tool.name)).toContain(
			"query_sdk",
		);
		...
		await enqueueMessage(messageFor(org.id), {
			...
			catalog: catalogFor(
				claudeModule({
					buildCredentialPlaceholder: () =>
						"lobu_secret_11111111-2222-3333-4444-555555555555",
				}),
			),
			mcp: mcpFixture().mcp,
		});
	...
	it("uses provider-owned metadata for a model missing from the bundled registry", async () => {
		...
		const module = Object.assign(tokenEchoingModule(), {
		...
		expect(metadata).toHaveBeenCalledWith(
			AGENT_ID,
			"synthetic-new-model",
			expect.objectContaining({ organizationId: org.id, userId: "user-turn" }),
		);
	});
	...
	it.each([
		"failure",
		"invalid",
	])("retains registry defaults when provider metadata is %s", async (mode) => {
		const org = await createTestOrganization();
		const module = Object.assign(tokenEchoingModule(), {
	...
	it("never asks the provider about a model the registry already carries", async () => {
		...
		const registryModel = getModel(
			"anthropic" as never,
			"claude-sonnet-4-5-20250929" as never,
		) as { contextWindow?: number } | undefined;
		expect(run.action_input.turn.compaction.context_window).toBe(
	...
	it("passes native tool history and provider metadata to Pi without gateway rewriting", async () => {
		...
		const snapshot = [
			...
			entry("t1", {
				...
				toolCallId: "toolu_01",
				toolName: "query_sdk",
				content: [{ type: "text", text: "3 entities" }],
				isError: false,
				timestamp: 1,
			}),
			entry("a2", {
				...assistant([{ type: "text", text: "There are 3." }]),
				stopReason: "stop",
			}),
    ...
    `;
		...
		const [run] = await agentTurnRuns();
		expect(
			(run.action_input.turn as Record<string, unknown>).session_jsonl,
		).toBe("");
		const claimed = await (
			await pollFleet("fleet-native-history", { agent_turn: true })
		).json();
    ...
    `;
		...
		const claimed = await (
			await pollFleet("fleet-native-compacted", { agent_turn: true })
		).json();
		...
		expect(turn.memory_flush).toMatchObject({
			enabled: true,
	...
	it("reads the latest native snapshot when a queued turn is claimed", async () => {
		...
		const secondSource = await admittedMessage({
			...messageFor(org.id),
		...
		const firstClaim = await pollFleet("fleet-native-first", {
			agent_turn: true,
		});
	...
	it("trims the persisted session to its latest compaction when it outgrows the cap", async () => {
		...
		await enqueueAgentTurn(
			await admittedMessage(messageFor(org.id)),
			dependencies,
		);
		await enqueueAgentTurn(
			await admittedMessage({ ...messageFor(org.id), messageId: "after-trim" }),
			dependencies,
		);
		const [first, second] = await agentTurnRuns();
		expect(
			(await (await pollFleet("fleet-oversize", { agent_turn: true })).json())
				.run_id,
		).toBe(first.id);
		const message = (
			id: string,
		...
		const before = Array.from({ length: 10 }, (_v, i) => [
			message(
				...
				i === 0 ? null : `a${i - 1}`,
				"user",
				i === 3 ? "x".repeat(4 * 1024 * 1024) : `question ${i}`,
			),
		...
		const after = [
			message("u10", "c1", "user", "what is the isolate lane?"),
			message("a10", "u10", "assistant", "the isolate answer"),
		];
		const completed = await postAsFleet("/api/workers/complete-agent-turn", {
			run_id: first.id,
			worker_id: "fleet-oversize",
			status: "completed",
			text: "the isolate answer",
		...
		const trimmed = nativeSession([
			{ ...before[16]!, parentId: null },
			...before.slice(17),
			compaction,
			...after,
		]);
		const [snapshot] =
			await sql`SELECT snapshot_jsonl, byte_size FROM agent_transcript_snapshot WHERE run_id = ${first.id}`;
		...
		const [reply] =
			await sql`SELECT action_input FROM runs WHERE queue_name = 'thread_response' AND action_input->>'finalText' = 'the isolate answer'`;
		expect(reply.action_input).toMatchObject({
			conversationId: "conv-turn",
			finalText: "the isolate answer",
		});
		// And the next turn resumes from it rather than failing the same way.
		const claimed = await (
			await pollFleet("fleet-after-trim", { agent_turn: true })
		).json();
		expect(claimed.run_id).toBe(second.id);
	...
	it("resets the native session when an oversize snapshot has no compaction to trim to", async () => {
		...
		const [snapshot] =
			await sql`SELECT byte_size FROM agent_transcript_snapshot WHERE run_id = ${first.id}`;
		expect(snapshot.byte_size).toBe(0);
		const claimed = await (
			await pollFleet("fleet-after-reset", { agent_turn: true })
		).json();
		expect(claimed.run_id).toBe(second.id);
	...
	it("carries the platform identity block the platform adapter registered", async () => {
		...
		await enqueueMessage(
			{
				...messageFor(org.id),
				platform: "slack",
				platformMetadata: { connectionId: "conn-slack-1" },
			},
			{
				agentSettings: settingsStore,
				catalog: catalogFor(claudeModule()),
		...
		const [run] = await agentTurnRuns();
		const prompt = (run.action_input.turn as { system_prompt: string })
			.system_prompt;
	...
	it("rolls back the claim when its native snapshot cannot be read", async () => {
		...
		const unavailable = new Proxy(realDb, {
			get(target, property) {
				if (property === "begin")
					return (fn: (tx: db.DbClient) => Promise<unknown>) =>
						target.begin((tx) =>
							fn(
								new Proxy(tx, {
									apply(query, thisArg, args: unknown[]) {
										...
										if (
											...
											parts
												.join("")
												.includes("FROM public.agent_transcript_snapshot")
										) {
											attemptedRead = true;
		...
		try {
			const response = await pollFleet("fleet-snapshot-unavailable", {
				agent_turn: true,
			});
	...
	it("repeats durable pending inputs after a lost heartbeat response", async () => {
		...
		const { run_id } = await (
			await pollFleet("fleet-repeat-input", { agent_turn: true })
		).json();
		await enqueueMessage(
			{
				...first,
				messageId: "durable-follow-up",
				messageText: "keep this input",
			},
			deps,
		);
	...
	it("offers a follow-up whose transient context differs, and carries that context", async () => {
		...
		await enqueueMessage(first, deps);
		const { run_id } = await (
			await pollFleet("fleet-transient-context", { agent_turn: true })
		).json();
		await enqueueMessage(
			{
				...first,
				messageId: "context-follow-up",
				messageText: "and this too",
				ephemeralContext:
		...
		expect(owner.action_input.turn.ephemeral_context).not.toBe(
			follower.action_input.turn.ephemeral_context,
		);
		const response = await postAsFleet("/api/workers/heartbeat", {
			run_id,
			worker_id: "fleet-transient-context",
		});
		expect(await response.json()).toMatchObject({
			continue: true,
			steer: [
	...
	it("offers a follow-up with no transient context without inventing one", async () => {
		...
		await enqueueMessage(first, deps);
		const { run_id } = await (
			await pollFleet("fleet-no-context", { agent_turn: true })
		).json();
		await enqueueMessage(
			{
				...first,
				messageId: "bare-follow-up",
				messageText: "no context here",
		...
		const response = await postAsFleet("/api/workers/heartbeat", {
			run_id,
			worker_id: "fleet-no-context",
		});
		const body = (await response.json()) as {
			steer?: Array<Record<string, unknown>>;
		};
		expect(body.steer?.[0]).toMatchObject({
			run_id: follower.id,
			message_id: "bare-follow-up",
		});
	...
	it("does not offer a follow-up admitted under a different model", async () => {
		...
		await enqueueMessage(first, deps);
		const { run_id } = await (
			await pollFleet("fleet-policy-model", { agent_turn: true })
		).json();
		await enqueueMessage(
			{
				...first,
				messageId: "other-model-follow-up",
				agentOptions: { model: "claude/claude-sonnet-4-5" },
			},
			deps,
		);
		const response = await postAsFleet("/api/workers/heartbeat", {
			run_id,
			worker_id: "fleet-policy-model",
		});
	...
	it("does not offer an input whose signed credential belongs to another run", async () => {
		...
		await enqueueMessage(first, deps);
		const { run_id } = await (
			await pollFleet("fleet-input-token-scope", { agent_turn: true })
		).json();
      ...
      (SELECT action_input->'credential' FROM runs WHERE id = ${owner.id})) WHERE id = ${follower.id}`;
		const response = await postAsFleet("/api/workers/heartbeat", {
			run_id,
			worker_id: "fleet-input-token-scope",
		});
		expect(await response.json()).toEqual({ continue: true });
		expect((await runRow(follower.id)).status).toBe("pending");
	});
	...
	async function inputScenario(texts = ["same text", "same text"], base = "") {
		...
		const sql = getTestDb();
		if (base) {
			const prior = await admittedMessage({
				...first,
				messageId: "prior-source",
			});
		...
		for (const [index, text] of texts.entries()) {
			await enqueueMessage(
				{ ...first, messageId: `input-${index}`, messageText: text },
				deps,
			);
		}
		...
		const entries = [
				...
				: []),
			{
				type: "message",
				id: "current-user",
				parentId: base ? JSON.parse(base.trim().split("\n").at(-1)!).id : null,
	...
	it("commits identical-text inputs by distinct native identities, once", async () => {
		...
		const response = await postAsFleet(
			"/api/workers/complete-agent-turn",
			body,
		);
		expect(await response.json()).toEqual({ ok: true, status: "completed" });
		const rows =
			await sql`SELECT status, claimed_by, claimed_at, completed_at, output_tail, outcome, exit_reason, run_metadata, action_input
      FROM runs WHERE id IN (${followers[0].id}, ${followers[1].id}) ORDER BY id`;
		for (const [index, row] of rows.entries()) {
			expect(row).toMatchObject({
				...
				outcome: "scoreable",
				exit_reason: "ok",
				output_tail: body.text,
				run_metadata: {
					consumed_by_run_id: owner.id,
					session_entry_id: `input-entry-${index}`,
				},
			});
		...
		expect(
			await sql`SELECT id FROM agent_transcript_snapshot WHERE run_id = ${owner.id}`,
		).toHaveLength(1);
		const replies =
			await sql`SELECT action_input FROM runs WHERE queue_name = 'thread_response' AND action_input->>'messageId' = 'msg-turn'`;
		expect(replies).toHaveLength(1);
		expect(replies[0].action_input.processedMessageIds).toEqual([
			"msg-turn",
			"input-0",
		...
		expect(
			await sql`SELECT id FROM runs WHERE queue_name = 'internal:turn_timeout'`,
		).toHaveLength(0);
	...
	it.each([
		...
		if (kind === "duplicate-run")
			body.consumed_inputs[1].run_id = followers[0].id;
		if (kind === "duplicate-entry")
			body.consumed_inputs[1].session_entry_id = "input-entry-0";
		if (kind === "wrong-run") body.consumed_inputs[0].run_id = body.run_id;
		if (kind === "wrong-entry")
			body.consumed_inputs[0].session_entry_id = "missing-entry";
		if (kind === "wrong-text")
			body.session_jsonl = body.session_jsonl.replace(
				"same text",
				"different text",
			);
		if (kind === "out-of-order") body.consumed_inputs.reverse();
		if (kind === "initial-entry")
			body.consumed_inputs[0].session_entry_id = "current-user";
		if (kind === "abandoned-branch")
			body.session_jsonl +=
				JSON.stringify({
					type: "message",
					id: "new-branch",
					...
					message: { role: "assistant", content: "different branch" },
				}) + "\n";
		const response = await postAsFleet(
			"/api/workers/complete-agent-turn",
			body,
		...
		expect((await agentTurnRuns()).map((run) => run.status)).toEqual([
			"failed",
			"pending",
			"pending",
		]);
	...
	it.each([
		...
		const base = nativeSession()
			.replace("what is the isolate lane?", "same text")
			.replace('"version":3', kind === "older" ? '"version":2' : '"version":3');
		...
		expect((await beat.json()).steer?.length ?? 0).toBe(
			kind === "older" ? 0 : 2,
		);
		if (kind === "pre-existing-entry")
			body.consumed_inputs[0].session_entry_id = "native-user";
		if (kind === "wrong-session")
			body.session_jsonl = body.session_jsonl.replace(
				'"id":"native-session"',
				'"id":"different-session"',
			);
		if (kind === "missing-boundary")
			body.session_jsonl = body.session_jsonl.replaceAll(
				"native-answer",
				"renamed-base-entry",
			);
		if (kind === "compaction")
			body.session_jsonl +=
				JSON.stringify({
					type: "compaction",
					id: "compact-inputs",
					parentId: "input-entry-1",
					timestamp: "2026-01-01T00:00:00.000Z",
					firstKeptEntryId: "input-entry-1",
					summary: "Earlier input summarized",
					tokensBefore: 100,
				}) + "\n";
		const response = await postAsFleet(
			"/api/workers/complete-agent-turn",
		...
		expect((await response.json()).status).toBe(
			["current", "compaction"].includes(kind) ? "completed" : "failed",
		);
	});
	...
	it("rolls back owner, input receipts, snapshot and delivery together", async () => {
						...
						target.begin((tx) =>
							fn(
								new Proxy(tx, {
									apply(query, thisArg, args: unknown[]) {
										if (
											Array.from(args[0] as TemplateStringsArray)
												.join(" ")
												.includes("outcome = 'scoreable'")
										)
											throw new Error("synthetic input commit failure");
										return Reflect.apply(query, thisArg, args);
									},
								}),
							),
		...
		try {
			expect(
				(await postAsFleet("/api/workers/complete-agent-turn", body)).status,
			).toBe(500);
		} finally {
			spy.mockRestore();
		}
		...
		expect(
			await sql`SELECT id FROM runs WHERE queue_name = 'thread_response'`,
		).toHaveLength(0);
		expect(
			(
				await (
					await postAsFleet("/api/workers/complete-agent-turn", body)
				).json()
			).status,
		).toBe("completed");
	});
	...
	it.each([
		...
		expect(await sweepStaleAgentTurnRuns(60)).toEqual({
			reaped: 0,
			delivered: 0,
		});
		if (terminal === "timeout") {
		...
		const ready =
			await sql`SELECT id, run_at > now() - interval '10 seconds' AS fresh FROM runs
		...
		expect(
			(await (await pollFleet("fleet-released", { agent_turn: true })).json())
				.run_id,
		).toBe(followers[1].id);
	});
	...
	it("expires an unresponsive cancellation despite heartbeats and fences late reports", async () => {
		...
		expect(
			(await (await pollFleet("fleet-successor", { agent_turn: true })).json())
				.run_id,
		).toBe(followers[0].id);
	...
	it("delivers pending cancellation and malformed dispatch errors atomically", async () => {
		...
		await cancelAgentTurn(
			await admittedMessage({
				...first,
				messageId: "cancel-pending",
				messageText: "/cancel",
			}),
		);
		expect((await runRow(followers[0].id)).status).toBe("cancelled");
		const next = await (
			await pollFleet("fleet-malformed-response", { agent_turn: true })
		).json();
		await failClaimedWorkerRun({
			runId: next.run_id,
			workerId: "fleet-malformed-response",
			errorMessage: "incomplete envelope",
		});
		const replies =
			await sql`SELECT action_input FROM runs WHERE queue_name = 'thread_response' ORDER BY id`;
		expect(replies.map((row) => row.action_input.processedMessageIds)).toEqual([
			["msg-turn"],
			["input-0"],
			["input-1"],
		]);
		expect(replies.slice(1).map((row) => row.action_input.error)).toEqual([
	...
	it("rechecks a refreshed startup deadline after the reaper candidate read", async () => {
		...
		const realDb = db.getDb();
		let refreshed = false;
		const racingDb = new Proxy(realDb, {
			apply(query, thisArg, args: unknown[]) {
				...
				if (
					...
					Array.from(args[0] as TemplateStringsArray)
						.join(" ")
						.startsWith("SELECT r.id FROM runs r WHERE")
				) {
	...
	it("serializes admission with an empty terminal handoff", async () => {
		...
		const [pending] =
			await sql`SELECT run_at > now() - interval '10 seconds' AS fresh FROM runs WHERE run_type = 'agent_turn' AND status = 'pending'`;
		expect(pending.fresh).toBe(true);
		const next = await (
			await pollFleet("fleet-concurrent-input", { agent_turn: true })
		).json();
	...
	it("upgrades an older base alone and offers followers on the next current-version turn", async () => {
		const { body, followers } = await inputScenario(
			["second", "third"],
			nativeSession().replace('"version":3', '"version":2'),
		);
		body.session_jsonl =
			body.session_jsonl
				.trim()
				.split("\n")
				.filter((line) => !JSON.parse(line).id?.startsWith("input-entry-"))
				.join("\n") + "\n";
		...
		const next = await (
			await pollFleet("fleet-current-base", { agent_turn: true })
		).json();
		expect(next.payload.turn.session_jsonl).toBe(body.session_jsonl);
		const beat = await postAsFleet("/api/workers/heartbeat", {
			run_id: next.run_id,
			worker_id: "fleet-current-base",
		});
		expect((await beat.json()).steer).toEqual([
			{ run_id: followers[1].id, message_id: "input-1", text: "third" },
		]);
	});
	...
	it.each([
		...
		"bytes",
		"single-large",
	])("keeps inputs beyond the %s offer bound intact", async (kind) => {
		const texts =
			kind === "count"
				? Array.from({ length: 34 }, (_, index) => `input ${index}`)
				: kind === "bytes"
					? ["🌍".repeat(12000), "🌍".repeat(12000), "intact tail"]
					: ["界".repeat(24000), "intact tail"];
		...
		expect(offered.length).toBe(
			kind === "count" ? 32 : kind === "bytes" ? 1 : 0,
		);
		expect(Buffer.byteLength(JSON.stringify(offered))).toBeLessThanOrEqual(
			65536,
		);
		const rows =
			await sql`SELECT action_input->'turn'->>'message_text' AS text FROM runs WHERE id > ${body.run_id} AND run_type = 'agent_turn' ORDER BY id`;
		...
		expect(
			(await (await pollFleet("fleet-bound-next", { agent_turn: true })).json())
				.run_id,
		).toBe(followers[0].id);
	});
	...
	it.each([
		...
		if (kind === "organization")
			changes = { organizationId: (await createTestOrganization()).id };
		const deps = {
			agentSettings: settingsStore,
			catalog: catalogFor(tokenEchoingModule()),
			gatewayUrl: GATEWAY_URL,
		...
		await enqueueMessage(first, deps);
		const { run_id } = await (
			await pollFleet("fleet-barrier", { agent_turn: true })
		).json();
		await enqueueMessage(
			{ ...first, ...changes, messageId: "barrier" } as MessagePayload,
			deps,
		);
		await enqueueMessage({ ...first, messageId: "after-barrier" }, deps);
		const response = await postAsFleet("/api/workers/heartbeat", {
			run_id,
			worker_id: "fleet-barrier",
		});
		const offered = (await response.json()).steer ?? [];
		if (["organization", "agent", "conversation"].includes(kind)) {
			expect(
				offered.map((input: { message_id: string }) => input.message_id),
			).toEqual(["after-barrier"]);
	...
	it("cancels a native turn on explicit cancel admission instead of steering", async () => {
		...
		const { run_id: runId } = await (
			await pollFleet("fleet-explicit-cancel", { agent_turn: true })
		).json();
		const cancel = await admittedMessage({
			...first,
		...
		const response = await postAsFleet("/api/workers/heartbeat", {
			worker_id: "fleet-explicit-cancel",
			run_id: runId,
		});
		expect(response.status).toBe(200);
		expect(await response.json()).toMatchObject({
	...
	it.each([
		...
		{
			name: "control metadata without text",
			messageText: "",
			platformMetadata: { control: "cancel" },
		},
		{
			name: "intent metadata without text",
			messageText: "",
		...
		for (const status of ["pending", "claimed", "running"]) {
			const first = {
				...messageFor(org.id),
				messageId: `original-${status}`,
				conversationId: `cancel-${status}`,
			};
			await enqueueMessage(first, {
				agentSettings: settingsStore,
				catalog: catalogFor(tokenEchoingModule()),
			...
			const cancel = await admittedMessage({
				...first,
				...command,
				messageId: `cancel-${status}`,
			});
			expect(await cancelAgentTurn(cancel)).toBe(true);
			const [row] =
				await sql`SELECT status, completed_at, exit_reason, run_metadata FROM runs WHERE id = ${run.id}`;
			if (status === "pending") {
				expect(row).toMatchObject({
					status: "cancelled",
	...
	it("keeps cancellation within its organization, agent, conversation and posting user", async () => {
		...
		const variants = [
			first,
			{ ...first, organizationId: otherOrg.id },
			{ ...first, agentId: "other-selected-agent" },
			{ ...first, conversationId: "other-conversation" },
			{ ...first, userId: "other-user" },
		];
		for (const [i, message] of variants.entries()) {
			await enqueueMessage(
				{ ...message, messageId: `scope-${i}` },
				{
					agentSettings: settingsStore,
					catalog: catalogFor(tokenEchoingModule()),
					gatewayUrl: GATEWAY_URL,
	...
	it("controls the active turn before queued work and consumes duplicate cancellation across workers", async () => {
		...
		await enqueueMessage(first, deps);
		const { run_id: activeId } = await (
			await pollFleet("fleet-control-active", { agent_turn: true })
		).json();
		await enqueueMessage({ ...first, messageId: "queued-after-active" }, deps);
		const queued = (await agentTurnRuns())[1];
		const cancel = await admittedMessage({
			...first,
			messageId: "replayed-cancel",
			messageText: "/cancel",
		});
		expect(
			await Promise.all([cancelAgentTurn(cancel), cancelAgentTurn(cancel)]),
		).toEqual([true, true]);
		...
		expect(
			(
				await (
					await pollFleet("fleet-control-next", { agent_turn: true })
				).json()
			).run_id,
		).toBeUndefined();
		await postAsFleet("/api/workers/complete-agent-turn", {
			run_id: activeId,
			worker_id: "fleet-control-active",
			status: "failed",
			error: "cancelled",
		});
		expect(
	...
	it("does not let a delayed cancellation stop a turn admitted after it", async () => {
		...
		expect(
			(
				await (
					await pollFleet("fleet-after-delayed-cancel", { agent_turn: true })
				).json()
			).run_id,
		).toBe(later.id);
		expect(await cancelAgentTurn(cancel)).toBe(true);
		const heartbeat = await postAsFleet("/api/workers/heartbeat", {
			run_id: later.id,
			worker_id: "fleet-after-delayed-cancel",
		});
	...
	it("propagates database failures and refuses an unbound cancellation receipt", async () => {
		...
		const cancel = await admittedMessage({
			...first,
			messageId: "retryable-cancel",
			messageText: "/cancel",
		});
		const spy = vi.spyOn(db, "getDb").mockImplementationOnce(() => {
			throw new Error("synthetic database failure");
		});
		try {
			await expect(cancelAgentTurn(cancel)).rejects.toThrow(
				"synthetic database failure",
			);
		} finally {
			spy.mockRestore();
		}
		await expect(cancelAgentTurn({ ...cancel, runId: 0 })).rejects.toThrow(
			"admitted message run ID",
		);
		await expect(
			cancelAgentTurn({ ...cancel, messageId: "wrong-message" }),
		).rejects.toThrow("no matching admitted message");
		expect((await agentTurnRuns())[0].status).toBe("pending");
		expect(await cancelAgentTurn(cancel)).toBe(true);
	...
	it("preserves a completion committed while cancellation waits for the active row", async () => {
		...
		await enqueueMessage(first, deps);
		const { run_id: activeId } = await (
			await pollFleet("fleet-completing-control", { agent_turn: true })
		).json();
		await enqueueMessage(
			{ ...first, messageId: "queued-after-completion" },
			deps,
		);
		const queued = (await agentTurnRuns())[1];
		const cancel = await admittedMessage({
			...first,
			messageId: "completion-race-cancel",
			messageText: "/cancel",
		});
		let locked!: () => void;
		...
		expect(await runRow(activeId)).toMatchObject({
			status: "completed",
			exit_reason: "ok",
		});
		expect((await runRow(queued.id)).status).toBe("pending");
	...
	it("carries a pinned sandbox as signed token claims and a remote-bash marker", async () => {
		...
		expect(turn.system_prompt).not.toContain(
			"The in-memory environment has no network access",
		);
		// The exec route trusts only the signed token for these — never a body.
		const claims = verifyWorkerToken(
			(run.action_input as { credential: string }).credential,
	...
	it("marks no remote bash for an unpinned conversation", async () => {
		...
		const [run] = await agentTurnRuns();
		expect(
			(run.action_input.turn as { tools?: { remote_runtime?: unknown } }).tools
				?.remote_runtime,
		).toBeUndefined();
	});
	...
	it("serializes concurrent fleet claims for one conversation until completion", async () => {
		...
		const responses = await Promise.all(
			workers.map((worker) => pollFleet(worker, { agent_turn: true })),
		);
		...
		expect(
			claims.filter((claim) => claim.run_id).map((claim) => claim.run_id),
		).toEqual([first.id]);
		expect((await runRow(second.id)).status).toBe("pending");

		const owner =
			workers[claims.findIndex((claim) => claim.run_id === first.id)];
		...
		expect(
			(await (await pollFleet("fleet-claim-next", { agent_turn: true })).json())
				.run_id,
		).toBe(second.id);
	});
	...
	it("does not skip a locked earlier turn to claim its later sibling", async () => {
		...
		await sql.begin(async (tx) => {
			...
			const response = await pollFleet("fleet-locked-claim", {
				agent_turn: true,
			});
			expect(response.status).toBe(200);
		...
		expect(
			(
				await (
					await pollFleet("fleet-after-unlock", { agent_turn: true })
				).json()
			).run_id,
		).toBe(first.id);
	});
	...
	it.each([
		"before",
		"after",
	])("serializes an earlier insert committed %s the locked recheck", async (position) => {
		const org = await createTestOrganization();
		await enqueueMessage(messageFor(org.id), {
			agentSettings: settingsStore,
			catalog: catalogFor(claudeModule()),
		...
		const earlierId = new Promise<number>((resolve) => {
			inserted = resolve;
		});
      ...
      `;
										...
										if (
											...
											return Promise.resolve(result).then(async (rows) => {
												...
												if (position === "after") {
													...
													const response = await pollFleet(
														"fleet-during-recheck",
														{ agent_turn: true },
													);
													expect(response.status).toBe(200);
        ...
        RETURNING id
      `;
			const response = await pollFleet("fleet-insert-race", {
				agent_turn: true,
			});
			expect(response.status).toBe(200);
			...
			expect(
				(await agentTurnRuns()).filter((run) => run.status === "pending"),
			).toHaveLength(1);
			const complete = await postAsFleet("/api/workers/complete-agent-turn", {
				run_id: claimedId,
				worker_id: "fleet-insert-race",
				status: "completed",
				session_jsonl: nativeSession(),
				text: "finished",
			});
			...
			expect(
				(
					await (
						await pollFleet("fleet-after-insert", { agent_turn: true })
					).json()
				).run_id,
	...
	it.each([
      ...
      `;
		expect(
			(
				await (
					await pollFleet("fleet-before-reap", { agent_turn: true })
				).json()
			).run_id,
		).toBeUndefined();
		expect(await sweepStaleAgentTurnRuns(60)).toEqual({
		...
		expect(
			(await (await pollFleet("fleet-after-reap", { agent_turn: true })).json())
				.run_id,
		).toBe(second.id);
	});
	...
	it.each([
        ...
        WHERE id = ${run.id}
      `;
		const response = await pollFleet("fleet-invalid-scope", {
			agent_turn: true,
		});
		expect(response.status).toBe(200);
		expect(await response.json()).toMatchObject({
			skipped_run_id: run.id,
			error: "agent turn run has an incomplete execution envelope",
		});
	...
	it.each([
		...
		expect(
			(await (await pollFleet("fleet-waiting", { agent_turn: true })).json())
				.run_id,
		).toBeUndefined();
	...
	it("claims distinct organization, agent and conversation scopes independently", async () => {
		...
		const claims = await Promise.all(
			runs.map(async (_, index) =>
				(await pollFleet(`fleet-scope-${index}`, { agent_turn: true })).json(),
			),
		);
		expect(claims.map((claim) => claim.run_id).sort()).toEqual(
	...
	it("a fleet worker that advertises the lane claims it and receives the turn plus the credential", async () => {
		...
		await enqueueMessage(
			{ ...messageFor(org.id), messageId: "msg-other" },
			{
				agentSettings: settingsStore,
				catalog: catalogFor(claudeModule()),
				gatewayUrl: GATEWAY_URL,
		...
		const other = (await agentTurnRuns()).find(
			(candidate) => candidate.id !== run.id,
		);
		expect(other).toBeDefined();
		expect(
			verifyWorkerToken(other?.action_input.credential as string)?.runId,
		).not.toBe(run.id);
	...
	it("a fleet worker without the capability leaves the run pending", async () => {
		...
		const response = await pollFleet("fleet-old-daemon", {
	...
	it("names the misconfiguration when the agent cannot run, instead of dropping the message", async () => {
		...
		expect(await enqueueMessage(noModel, deps)).toBe(
		...
		expect(
			AGENT_ERRORS[AgentErrorCode.NO_MODEL_CONFIGURED].message,
		).toBeTruthy();
	});
	...
	it.each([
		...
		expect(run.action_input.turn.provider.base_url).toContain(
			"gateway.test.invalid",
		);
	});
	...
	it.each([
		"model",
		"window tools",
	] as const)("delivers the %s misconfiguration to the client, not a deadline timeout", async (kind) => {
		const sql = getTestDb();
		const message =
			kind === "model"
				? messageFor((await createTestOrganization()).id)
				: await automationMessage();
		const organizationId = message.organizationId!;
		message.agentOptions =
			kind === "model"
				? {}
				: { ...message.agentOptions, toolsConfig: { strictMode: true } };
			...
			kind === "model"
				...
				: {
						error: expect.stringContaining(
							"tool policy excludes query_sdk, run_sdk",
						),
					},
		);
		expect(await agentTurnRuns()).toHaveLength(0);
		...
		expect(
			await failTurnIfPending(deploymentName, message.messageId, unrunnable!),
		).toBe(true);
		expect(
			await failTurnIfPending(deploymentName, message.messageId, unrunnable!),
		).toBe(false);
    ...
    `) as unknown as Array<{ n: number }>;
		...
		expect(delivered[0]!.errorCode).not.toBe(
			AgentErrorCode.WORKER_UNRESPONSIVE,
		);
		expect(String(delivered[0]!.error)).toContain(
			kind === "model" ? "model" : "tool policy excludes",
		);
	});
	...
	it.each([
		"application/pdf",
		"image/png",
	])("claims an envelope with bounded %s attachment metadata", async (mimetype) => {
		const org = await createTestOrganization();
		const message = messageFor(org.id);
		message.platformMetadata = {
			files: [
		...
		const [run] = await agentTurnRuns();
		expect(
			Value.Check(AgentTurnPollPayloadSchema, { turn: run.action_input.turn }),
		).toBe(true);
		const response = await pollFleet("fleet-attachment-boundary", {
			agent_turn: true,
		});
		expect(response.status).toBe(200);
		const body = await response.json();
		...
		expect(body.payload.turn.message_images).toEqual([
			{ mime_type: "image/png", data: Buffer.from("PNG!").toString("base64") },
		]);
		const [claimed] =
			await getTestDb()`SELECT status, claimed_by FROM runs WHERE id = ${run.id}`;
		expect(claimed).toMatchObject({
			status: "running",
			claimed_by: "fleet-attachment-boundary",
		});
	});
	...
	it("enqueues an attachment-only message once its image resolves, and refuses one whose image does not", async () => {
		...
		withImage.platformMetadata = {
			files: [
				{ id: "art-image", name: "shot.png", mimetype: "image/png", size: 4 },
			],
		};
		...
		const secondTurn = (
			second?.action_input as { turn: Record<string, unknown> }
		).turn;
		expect(secondTurn.message_images).toBeUndefined();
		expect(secondTurn.message_files).toEqual([
	...
	it("puts the model's own modalities on the envelope, from pi-ai's registry", async () => {
		...
		const [run] = await agentTurnRuns();
		const turn = (
			run?.action_input as { turn: { provider: { input?: string[] } } }
		).turn;
	...
	it("carries the registry's reasoning support and output ceiling for a known model", async () => {
		...
		const registryModel = getModel(
			"anthropic" as never,
			"claude-sonnet-4-5-20250929" as never,
		) as { reasoning?: boolean; maxTokens?: number } | undefined;
	...
	it("carries ephemeralContext on the turn-scoped channel, NOT the durable message", async () => {
		...
		await enqueueMessage(
			{
				...messageFor(org.id),
				ephemeralContext: "the deploy is frozen until Friday",
			},
			{
				agentSettings: settingsStore,
				catalog: catalogFor(tokenEchoingModule()),
		...
		const history = nativeSession([
			{
				...
				message: {
					role: "user",
					content: "history-the-user-asked-to-drop",
					timestamp: 1,
				},
			},
		]);
    ...
    `;
		...
		const claimed = await (
			await pollFleet("fleet-reset", { agent_turn: true })
		).json();
    ...
    `;
		...
		const claimed = await (
			await pollFleet("fleet-no-reset", { agent_turn: true })
		).json();
...
describe("agent turn completion", () => {
	...
	it("stamps the tool ledger onto the terminal reply the guardrail reads", async () => {
		...
		const response = await postAsFleet("/api/workers/complete-agent-turn", {
			...
			stop_reason: "stop",
			session_jsonl: nativeSession(),
			tools_used: ["query_sdk", "suggest_actions"],
			exit_reason: "ok",
		});
		expect(response.status).toBe(200);
	...
	it("persists native IDs, tool pairs and summaries verbatim under the completion fence", async () => {
		...
		const session = nativeSession([
			{
				...
				id: "u1",
				parentId: null,
				message: { role: "user", content: "count" },
			},
			{
				type: "message",
				id: "a1",
			...
			},
			{
				...
				message: {
					role: "assistant",
					content: [{ type: "text", text: "there are 3" }],
					stopReason: "stop",
				},
			},
			{
				...
				parentId: "a2",
				customType: "lobu.memory_flush_state",
				data: { compactionCount: 0, outcome: "stored" },
			},
			{
				type: "compaction",
				id: "c1",
		...
		const sql = getTestDb();
		const [snapshot] =
			await sql`SELECT snapshot_jsonl, byte_size FROM agent_transcript_snapshot WHERE run_id = ${runId}`;
		...
		const snapshots =
			await sql`SELECT snapshot_jsonl FROM agent_transcript_snapshot WHERE run_id = ${runId}`;
	...
	it("does not persist a stale completion after its lease changes during the request", async () => {
		...
		const stolenLease = new Proxy(realDb, {
			get(target, property) {
				if (property === "begin")
					return async (fn: (tx: db.DbClient) => Promise<unknown>) => {
						if (!leaseChanged) {
							leaseChanged = true;
	...
	it.each([
		...
		const sql = getTestDb();
		const [run] =
			await sql`SELECT organization_id FROM runs WHERE id = ${runId}`;
    ...
    `;
		...
		const snapshots =
			await sql`SELECT run_id, snapshot_jsonl FROM agent_transcript_snapshot`;
		expect(snapshots).toHaveLength(1);
		expect(snapshots[0]).toMatchObject({
			run_id: prior.id,
			snapshot_jsonl: previous,
		});
	...
	it("drops an oversize or malformed span unpublished and unacknowledged, and keeps the beat", async () => {
		...
		const malformed = await postAsFleet("/api/workers/heartbeat", {
			...
			worker_id: workerId,
			turn_tool_events: [
				{ tool_call_id: "c", name: "x", is_error: "yes", output: "o" },
			],
		});
		...
		expect(await fine.json()).toMatchObject({
	...
	it("streams an in-flight turn to the client on the heartbeat it already sends", async () => {
		...
		expect(rows.map((row) => row.delta).join("")).toBe(
			"the isolate lane answered",
		);
...
describe("agent turn reaper", () => {
	...
	it("reaps a stale turn and delivers its error, leaving a heartbeating turn alone", async () => {
		...
		expect(delivered!.action_input).toMatchObject({
			errorCode: AgentErrorCode.WORKER_DIED,
		});
	...
	it("a worker that completes between the candidate read and the timeout wins", async () => {
		...
		expect(replies).toHaveLength(1);
		expect(replies[0].action_input).toMatchObject({
			finalText: "made it just in time",
		});
	});
});
