...
function capabilityFromMeta(value: unknown, key: string): string | null {
	...
	return trimmed && trimmed.length <= MCP_APP_CAPABILITY_MAX_LENGTH
		? trimmed
		: null;
}
...
function supportsMcpApps(
	...
	const uiExtension = (extensions as Record<string, unknown>)[
...
function supportsAppSandboxDomain(
	...
	const visibility = (experimental as Record<string, unknown>)[
		"openai/visibility"
	];
	return (
		!!visibility &&
...
function createServerForContext(
	...
	server.setRequestHandler(ListToolsRequestSchema, async () => {
		...
		const publicOnly =
			!!authCtx.organizationId && !effectiveCrossOrg && !authCtx.memberRole;
		...
		const allTools = getMcpTools({
			...
			maxAccessLevel,
			adminScopeEligible:
				effectiveCrossOrg || isAdminOrOwnerRole(authCtx.memberRole),
		})
			.filter((t) => {
				const visibility = (t._meta?.ui as { visibility?: unknown } | undefined)
					?.visibility;
				return (
					mcpAppsSupported ||
					!(
	...
	server.setRequestHandler(CallToolRequestSchema, async (request) => {
		...
		// downgrade.
		const uiMemberRoleMeta: { "lobu/member-role": string | null } | undefined =
			tool?.mcpMeta?.ui
		...
		try {
			...
			const softError =
				isSoftErrorResult(publicResult) || attachedAuthChallenge;
			const uiResourceUri = (
			...
			const scopeChallenge =
				...
				!hasRequiredMcpScope(requiredAccess, authCtx.scopes)
					? buildMcpBearerChallenge(authCtx.requestUrl, {
							...
							errorDescription:
								error?.message ?? "The token lacks the required MCP scope.",
							scope: requiredScope,
						})
					: null;
...
async function buildSessionInstructions(
	...
	const labels = workspaces.map((workspace) => {
		const qualifiers = [workspace.personal ? "personal" : null].filter(
...
async function recoverSessionAuthContext(
	...
	authCtx.requestedAgentId =
		persisted.requestedAgentId ?? authCtx.requestedAgentId;
...
async function handleTransportRequest(
	...
	const response = await mcpRequestFormat.run({ rawJson }, () =>
...
function createSessionTransport(
	...
	const server = createServerForContext(
		env,
		authCtx,
		authCtx.supportsMcpApps ?? false,
	);
...
export async function handleMcp(
	...
	if (
		requestAuth.tokenType === "oauth" &&
		!requestAuth.scopedToOrg &&
		requestAuth.tokenOrganizationId &&
		!requestAuth.scopes?.includes("device_worker:run")
	) {
		return buildUnauthorizedResponse(
			req,
			"This OAuth authorization uses a retired default workspace. Reauthorize the client to select workspace grants without a default target.",
		);
	}
	...
	if (sessionId && sessions.has(sessionId)) {
		...
		const freshRequestAuthCtx = c.var.mcpIsAuthenticated
		...
		if (
			priorCapture !== freshCapture ||
			(priorCapture &&
				(session.authCtx.captureIdentity?.runId !==
					freshRequestAuthCtx?.captureIdentity?.runId ||
					session.authCtx.captureIdentity?.automationRunId !==
						freshRequestAuthCtx?.captureIdentity?.automationRunId))
		) {
			return buildJsonRpcErrorResponse(
				"MCP capture identity changed. Re-initialize.",
				null,
				400,
			);
		}
		if (
			session.authCtx.tokenType === "oauth" &&
			freshRequestAuthCtx?.tokenType !== "oauth"
		) {
			return buildUnauthorizedResponse(
				req,
		...
		if (freshRequestAuthCtx) {
			...
			if (freshCtx.agentId && freshCtx.agentId !== session.authCtx.agentId) {
				clearSession();
				return buildJsonRpcErrorResponse(
					"Session agent changed. Re-initialize.",
					null,
					400,
				);
			}
			...
			if (
				!session.authCtx.scopedToOrg &&
				freshCtx.organizationId !== session.authCtx.organizationId
			) {
				clearSession();
				return buildJsonRpcErrorResponse(
					"Session workspace binding changed. Re-initialize.",
					null,
					400,
				);
			}
			...
			session.authCtx.instructions = await buildSessionInstructions(
				session.authCtx,
			);
		}
	...
	if (req.method === "POST") {
		...
		authCtx.supportsAppSandboxDomain = supportsAppSandboxDomain(
