...
export async function runAutomationScriptTask(
  ...
  const [run] = await sql`
    ...
    JOIN automation_versions v
      ON v.id = (r.approved_input->>'version_id')::bigint
      AND v.automation_id = a.automation_group_id
  ...
  if (!run)
    throw new Error(
      'Automation script task has no correlated run and pinned version'
    );
  if (run.status !== 'running') return;
  const input = run.approved_input;
  if (
    input?.executor?.kind !== 'script' ||
    typeof input.executor.source !== 'string'
  ) {
  ...
  const context: AutomationScriptContext = {
    ...
    organization_slug: String(run.organization_slug),
  };
  const result = await executeAutomationScript({
    compiledScript: input.executor.source,
    params: input.executor.params,
    context,
  ...
  if (!result.success || error) {
    if (attempt >= 3 || reactionErrorIsNonTransient(error)) {
      await markAutomationRunFailed(
        sql,
        payload.sourceRunId,
        error ?? 'Automation script failed'
      );
      return;
    }
  ...
  await sql.begin(async (tx) => {
    ...
    `;
    if (!automation)
      throw new Error('Automation disappeared before script completion');
