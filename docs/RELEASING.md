...
The CLI also publishes four generated runtime packages at the same exact version: `@lobu/runtime-server`, `@lobu/runtime-device`, `@lobu/runtime-postgres`, and `@lobu/runtime-embeddings`. Its packaged runtime manifest pins these components, which commands install on demand. Include all four in artifact, provenance, and registry-availability checks; a published CLI alone does not establish that its runtime components are available.
...
Publication itself is `npm publish --tag canary`: the tag moves as part of the publish because a separate `npm dist-tag add` cannot authenticate through OIDC. The Linux `published-artifact-smoke` matrix is therefore a post-publication check, not a gate. If an artifact defect is confirmed, the canary is already on the registry and needs `npm deprecate` plus a newer canary. If the only failure is an exact runtime version returning 404 during registry propagation, wait for that version to become available, preserve the first failure, and rerun the unchanged failed smoke jobs.

For dogfooding and bug reports, pin the full version printed by the successful publication run and record `lobu --version`. A package manager can resolve a cached `canary` tag to an older build; an exact version removes that ambiguity. Keep stable `latest` unchanged when publishing a canary.
